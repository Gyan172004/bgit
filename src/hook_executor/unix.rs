...
pub fn execute_hook_util(
    ...
    let pre_event_hook_path_str = pre_event_hook_path.to_str().ok_or_else(|| {
        create_hook_error(
            "Invalid path",
    ...
    let metadata = fs::metadata(pre_event_hook_path).map_err(|e| {
        create_hook_error(
    ...
    if permissions.mode() & 0o111 == 0 {
        ...
        fs::set_permissions(pre_event_hook_path, permissions).map_err(|e| {
            create_hook_error(
